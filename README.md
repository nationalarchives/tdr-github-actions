...
The pull requests are created to a branch called `scala-steward-dependencies` using the tna-digital-archiving-jenkins user
...
Each of the repos contains a `mergify.yml` file that will auto merge the dependency prs to the `scala-steward-dependencies` branch if all the tests pass.
...
This ensures the `scala-steward-dependencies` branch is always inline with master/main so that scala-steward fetches the correct dependencies and so that the mergify bot can automatically merge PRs.

There is a `create-dependencies-pull-requests.yml` workflow that runs every week on Monday that will create a `scala dependencies` pr containing all the merged dependencies for all the repos.
These should be auto merged by Mergify if all the tests pass.
